...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def helper(match_para):
        lower_para  = match_para.lower()
        no_punc = lower_para.translate(str.maketrans('', '', string.punctuation))
        words = no_punc.split()
...
def accuracy(entered: str, source: str) -> float:
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    total = min(len(entered_words), len(source_words))
    
    if len(entered_words) == 0 and len(source_words) == 0:
        return 100.0
    
    if len(entered_words) and len(source_words) == 0:
        return 0.0

    if len(entered_words) == 0 and len(source_words):
        return 0.0
    ...
    for i in range(total):
        if entered_words[i] == source_words[i]:
            match_num += 1
    return ( match_num / len(entered_words) ) * 100
    # END PROBLEM 3
...
def autocorrect(entered_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    if entered_word in word_list:
        ...
        for item in word_list:
            ls = diff_function(entered_word, item, limit)
            if ls <= limit and ls < ans_num:
                ans_num = ls
                ans = item
        if ans == "":
            return entered_word
...
def furry_fixes(entered: str, source: str, limit: int) -> int:
    ...
    if limit < 0:
        ...
        if len(entered) == 0 and len(source) == 0:
            return 0
        elif len(entered) and len(source) == 0:
            return len(entered)
        elif len(entered) == 0 and len(source):
            return len(source)
        else:
    ...
    if limit < 0:
        ...
        if len(entered) == 0 and len(source) == 0:
            ...
            if entered[0] == source[0]:
                ...
                '''
                    主要是分为三种策略 Add Substitute Remove
                '''
                return 1 + min( minimum_mewtations( entered[1:], source[1:], limit - 1 ),#Substitute
                                minimum_mewtations( entered, source[1:], limit - 1 ), # Add
                                minimum_mewtations( entered[1:], source, limit - 1 ) # Remove
                           )
    ...
    for i in range(len(times)):
        ...
        for j in range(len(times[i])):
            ...
            for k in range(len(times)):
                if times[i][j] > times[k][j] or (times[i][j] == times[k][j] and i != k and i > k):
                    flag = False
            if flag == True:
                cur_fast.append(words[j])
        player.append(cur_fast)
    return player
    # END PROBLEM 10
