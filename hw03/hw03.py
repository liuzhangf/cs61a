...
def digit_distance(num):
    ...
    if num < 100 and num >= 10:
        ...
        return abs(last_digit - second_last_digit) + digit_distance(num // 10)
...
def interleaved_sum(num, f_odd, f_even):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(cur_num, is_odd):
        if cur_num <= num:
            if is_odd:
                return f_odd(cur_num) + helper(cur_num + 1, not is_odd)
            else :
                return f_even(cur_num) + helper(cur_num + 1,  not is_odd)
        else:
            return 0
...
def count_dollars(sum_needed):
    ...
    def helper(amount, bill):
        if amount == 0:
            ...
            return 0
        use = helper(amount - bill, bill)
        not_use = helper(amount, next_smaller_dollar(bill))
        return use + not_use
...
def count_dollars_upward(sum_needed):
    ...
    def helper(amount, bill):
        if amount == 0:
            ...
            return 0
        use = helper(amount - bill, bill)
        not_use = helper(amount, next_larger_dollar(bill))
        return use + not_use
...
def move_stack(num, start, end):
    ...
    print_move(start, end)
    print_move(start, middle)
    print_move(end, middle)
    print_move(start, end)
    move_stack(num - 1, middle , end)
...
def make_anonymous_factorial():
    ...
    True
    """
    return ( lambda f : lambda n : f(f, n) ) (lambda f, k : 1 if k == 1 else k * f(f, k - 1))
