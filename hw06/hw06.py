...
class VendingMachine:
    ...
    def vend(self) -> str:
        ...
        if self.num == 0:
            ...
            last_balance = self.balance - self.price
            self.balance = 0
            self.num -= 1
            return f'Here is your {self.product} and ${last_balance} change.'
        elif self.balance == self.price:
            self.balance = 0
            self.num -= 1
...
def store_digits(n: int):
    ...
    """
    "*** YOUR CODE HERE ***"
    link_list = Link.empty
    while n:
        last_digit = n % 10
        link_list = Link( last_digit ,link_list)
        n //= 10
    return link_list
...
def two_list(vals, counts):
    ...
    for i in range(len(counts)):
        for j in range(counts[i]):
            Link_list = Link(vals[i], Link_list)
    return Link_list
