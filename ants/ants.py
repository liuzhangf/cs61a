...
class Ant(Insect):
    ...
    def add_to(self, place: Place):
        ...
        if place.ant is None:
            ...
            elif place.ant.is_container == True:
    ...
    def double(self):
        ...
        if hasattr ( self, 'ant_contained' ) and self.ant_contained is not None:
...
D
        ...
        while cur_step < self.lower_bound and cur_loc.is_hive == False:
            cur_loc = cur_loc.entrance
            cur_step += 1            

        while cur_loc.is_hive == False and cur_loc.bees == [] and cur_step <= self.upper_bound:
            cur_loc = cur_loc.entrance
            cur_step += 1
        if cur_loc.is_hive == True:
            return None
        elif cur_step > self.upper_bound:
            return None
        return random_bee(cur_loc.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, damage_taken: float):
        ...
        for item in self.place.bees[:]:
            item.reduce_health(damage_taken)
        if self.health <= damage_taken:
            for item in self.place.bees[:]:
                item.reduce_health(self.damage)  
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate: GameState):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        cur_loc = self.place.exit
        while cur_loc is not None:
            if cur_loc.ant is not None:
                cur_loc.ant.double()
            cur_loc = cur_loc.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target: Bee | None):
        ...
        target.slow_remaining = 5

        def slower_action(gamestate : GameState):
            if target.slow_remaining > 0:
                if gamestate.time % 2 == 0:
                    target.original_action(gamestate)
                target.slow_remaining -= 1
            else :
                target.original_action(gamestate)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self) -> dict[Bee, int]:
        ...
        cur_loc = self.place
        dist = 0
        while cur_loc is not None and not cur_loc.is_hive:
            if cur_loc.ant is not None and cur_loc.ant is not self:
                dict[cur_loc.ant] = dist
            if cur_loc.bees is not None:
                for item in cur_loc.bees:
                    dict[item] = dist
            dist += 1
            cur_loc = cur_loc.entrance
        return dict
    ...
    def calculate_damage(self, distance: int) -> float:
        # BEGIN Problem EC 4
        damage = 2.0 - (distance * 0.25) - (self.insects_shot * 0.0625)
        return max(0, damage)
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def blocked(self) -> bool:
        ...
        # BEGIN Problem EC 3
        if self.place is not None and self.place.ant is not None:
            return self.place.ant.blocks_path == True
        
        if self.place is not None and self.place.ant is None:
            return False
        # END Problem EC 3
    ...
    def action(self, gamestate: GameState):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if hasattr(self, 'slow_remaining') and self.slow_remaining > 0 :
        ...
        if self.scare_turn > 0:
            if self.place.entrance is not None: 
                if not self.place.entrance.is_hive and self.place.entrance is not None:
