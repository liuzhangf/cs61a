...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        "*** YOUR CODE HERE ***"
        cur_frame = self
        while not symbol in cur_frame.bindings:
            if cur_frame.parent is not None:
    ...
    def make_child_frame(self, formals, vals):
        ...
        formals_item = formals
        vals_item = vals
        while formals_item is not nil and vals_item is not nil:
            new_Frame.define(formals_item.first, vals_item.first)
            formals_item = formals_item.rest
            vals_item = vals_item.rest
        return new_Frame
        # END PROBLEM 8
